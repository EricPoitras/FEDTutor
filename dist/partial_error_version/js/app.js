...
function initiate_session() {
	...
	try {
		...
		html.value = data.activities[0].code[0].snippet;
	} catch {
...
function update_livepreview() {
	code.open();
	code.writeln("<!DOCTYPE html><head><style>" + css.value + "</style>" + "<title>W3C Validation</title></head><body>" + html.value + "<script>" + js.value + "</script></body>");
	code.close();
}
...
function increment_activity() {
	...
	if (activity == 1) {
		...
		var pos = html.value.indexOf(data.activities[activity].code[0].insert_after) + data.activities[activity].code[0].insert_after.length;
		html.value = [html.value.slice(0, pos), "\n", data.activities[activity].code[0].snippet, html.value.slice(pos)].join("");
	} else if (activity == 2) {
		...
		var pos = html.value.indexOf(data.activities[activity].code[0].insert_after) + data.activities[activity].code[0].insert_after.length;
		html.value = [html.value.slice(0, pos), "\n", data.activities[activity].code[0].snippet, "\n", "    ", html.value.slice(pos).replace(/(\r\n|\n|\r)/gm, "")].join("");
		var pos2 = html.value.indexOf(data.activities[activity].code[1].insert_after) + data.activities[activity].code[1].insert_after.length;
		html.value = [html.value.slice(0, pos2), "\n", data.activities[activity].code[1].snippet, html.value.slice(pos2)].join("");
	} else if (activity == 3) {
		...
		var pos = html.value.indexOf(data.activities[activity].code[0].insert_after) + data.activities[activity].code[0].insert_after.length;
		html.value = [html.value.slice(0, pos), " ", data.activities[activity].code[0].snippet, html.value.slice(pos)].join("");
	} else if (activity == 4) {
		...
		var pos = html.value.lastIndexOf(data.activities[activity].code[0].insert_after) + data.activities[activity].code[0].insert_after.length;
		html.value = [html.value.slice(0, pos), data.activities[activity].code[0].snippet, html.value.slice(pos)].join("");
		var pos2 = html.value.lastIndexOf(data.activities[activity].code[1].insert_after) + data.activities[activity].code[1].insert_after.length;
		html.value = [html.value.slice(0, pos2), data.activities[activity].code[1].snippet, html.value.slice(pos2)].join("");
	} else if (activity == 7) {
...
function store_response(grade_response) {
	var myObj = {
		version: "FEDTutor_v1.0_build03/06/20_partial_error",
		session: session_id,
		username: username,
		gender: gender,
		age: age,
...
async function HTML_W3_Validation() {
	var body_post = String("<!DOCTYPE html><head><style>" + css.value + "</style>" + "<title>W3C Validation</title></head><body>" + html.value + "<script>" + js.value + "</script></body>");
